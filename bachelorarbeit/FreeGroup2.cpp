...
WORD operator*(WORD op1, WORD op2)//Gruppenoperation auf F_2, akzeptiert reduzierte Wörter, berechnet Ergebnis rekursiv und gibt Ergebnis in reduzierter Darstellung aus
{
	...
	if (!secondStartsWithA)//Entferne vorhergehende 0 wenn op2 mit b anfängt 
		op2.pop_front();
...
bool operator<(const WORD& op1, const WORD& op2) //Definiere lexikographische Ordnung auf F_2 um std::map nutzen zu können
{
